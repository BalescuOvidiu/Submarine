 ...
 *  @brief:
 */
void Component::move (sf::Vector2f vector) {
 ...
 *  @brief:
 */
void Component::movePoints (sf::Vector2f vector) {
 ...
 *  @brief:
 */
sf::Vertex& Component::operator [] (std::size_t index) {
...
void Component::addEllipse (
	...
	if (angleBegin > angleEnd) {
		angleBegin += angleEnd;
		angleEnd = angleBegin - angleEnd;
		angleBegin = angleBegin - angleEnd;
	}
...
void Component::rotate (double newAngle) {
	...
	newAngle = mapDegreesInFirstCircle (newAngle);
	...
	this->angle = mapDegreesInFirstCircle (this->angle + newAngle);
	this->update ();
}
